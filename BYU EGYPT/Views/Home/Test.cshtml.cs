...
namespace BYU_EGYPT.Views.Home
{
    public class TestModel : PageModel
    {
        ...
        public async Task<IActionResult> BoxCallback(string code)
        {
            ...
            var redirectUrl = "https://localhost:7164/";
        ...
        public async Task OnGetAsync()
        {
            ...
            //var config = new BoxConfig("5zbozuhwobclkx4irixpwt5rz7x5awkv", "HBcQ96QiJJcyGGQ38qeL2t8qBlemN9mb", new Uri(redirectUrl));
            //var sdk = new BoxClient(config);
            //authorizationUrl = "https://account.box.com/api/oauth2/authorize?client_id=5zbozuhwobclkx4irixpwt5rz7x5awkv&redirect_uri=https://localhost:7164/&response_type=code";
            //redirectTo(authorizationUrl);
            //string authorizationCode = HttpContext.Request.Query["code"];
            //var session = await sdk.Auth.AuthenticateAsync(authorizationCode);
            //var client = new BoxClient(config, session);
            // set up Box configuration with Developer Token
            var config = new BoxConfigBuilder("5zbozuhwobclkx4irixpwt5rz7x5awkv", "HBcQ96QiJJcyGGQ38qeL2t8qBlemN9mb", new Uri("https://localhost:7164")).Build();
            var session = new OAuthSession("UvhzExlPQ9qpYmCCn2NzmMiX7h4k4d9r", "N/A", 3600, "bearer"); // add developer token here
            var client = new BoxClient(config, session);
